...
- **Widening allowlist or rotating the primary** (`profile allow`, `profile allow-path`, `profile set-default-header`, `profile set-allow-http`, `profile set-secret`) — these require `--passphrase`, which is constant-time verified against a stored value. You don't have it. A wrong guess errors cleanly.
