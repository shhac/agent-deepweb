...
```bash
./dist/agent-deepweb-darwin-arm64 --version
./dist/agent-deepweb-darwin-arm64 usage
```
...
4. Ensure the `install` block installs shell completions — after `bin.install`, add
   `generate_completions_from_executable(bin/"agent-deepweb", "completion")` (the binary supports `completion bash|zsh|fish`)
5. Commit and push:
   ```bash
   cd ../homebrew-tap
